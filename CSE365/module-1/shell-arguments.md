...
Attacker: if a program takes your input and passes it directly to the shell — you control the argument. That's command injection. One malicious string and you're executing arbitrary code.

Defender: never trust user input. Sanitize everything before it touches a shell call. Better yet — avoid shell calls entirely.
...
Confusing the command with its arguments. Forgetting that spaces are the separator — a space in the wrong place breaks everything. Assuming arguments are safe because the user typed them.
